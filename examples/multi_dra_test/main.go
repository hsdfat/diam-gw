...
func main() {
	fmt.Println("╔══════════════════════════════════════════════════════════════╗")
	fmt.Println("║  Multi-DRA Priority-Based Client with Failover Testing      ║")
	fmt.Println("╚══════════════════════════════════════════════════════════════╝")
	fmt.Println()
	...
	// Configure DRA pool with 4 DRAs
	// 2 DRAs with priority 1 (highest)
	// 2 DRAs with priority 2 (backup)
	config := client.DefaultDRAPoolConfig()
	config.OriginHost = "client.example.com"
	config.OriginRealm = "example.com"
	config.ConnectionsPerDRA = 1 // 1 connection per DRA
	...
	// Start the pool (connects to ALL DRAs)
	...
	fmt.Println("Kill Priority 1 DRAs to test failover to Priority 2")
	fmt.Println("═══════════════════════════════════════════════════════════" + "\n")
...
func sendTestMessages(ctx context.Context, pool *client.DRAPool) {
	...
	for {
		...
		case <-ticker.C:
			...
			// Send via pool (will use active priority DRAs)
...
func displayConfig(config *client.DRAPoolConfig) {
	...
	fmt.Println("Priority Groups:")
	priorities := make(map[int][]string)
	for _, dra := range config.DRAs {
		priorities[dra.Priority] = append(priorities[dra.Priority], dra.Name)
	}
	for priority := 1; priority <= 10; priority++ {
		if dras, exists := priorities[priority]; exists {
...
func displayPoolStatus(pool *client.DRAPool) {
	...
	for _, name := range draNames {
		...
		// Check if this DRA is at active priority
