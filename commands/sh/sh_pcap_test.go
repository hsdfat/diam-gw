...
func TestPU_Pair_PCAP(t *testing.T) {
	...
	request.UserIdentity = &UserIdentity{
		PublicIdentity: ptrUTF8String("452040000000010"),
		Msisdn:         ptrOctetString([]byte{0x01, 0x02, 0x03}),
	}
	request.DataReference = []models_base.Enumerated{models_base.Enumerated(1)}
	request.UserData = models_base.OctetString([]byte{0x01, 0x02, 0x03})
	request.ServiceIndication = ptrOctetString([]byte{0x01, 0x02, 0x03})
	request.ExpiryTime = ptrTime(models_base.Time(time.Now()))
	...
	answer.ExpiryTime = ptrTime(models_base.Time(time.Now()))
	answer.RepositoryDataId = &RepositoryDataID{
		ServiceIndication: models_base.OctetString([]byte{0x01, 0x02, 0x03}),
		SequenceNumber:    models_base.Unsigned32(1),
	}
...
// TestPN_Pair_PCAP tests PCAP file generation for PN request-response pair
func TestPN_Pair_PCAP(t *testing.T) {
	// Create testdata directory
	if err := os.MkdirAll("testdata", 0755); err != nil {
		t.Fatalf("Failed to create testdata directory: %v", err)
	}
	...
	// Create pcap file path
	pcapFile := filepath.Join("testdata", "test_pnr_pna.pcap")
	// PCAP files are kept for Wireshark analysis

	// Create Request message
	request := NewPushNotificationRequest()
	request.SessionId = models_base.UTF8String("client.example.com;1234567890;1")
	request.VendorSpecificApplicationId = &VendorSpecificApplicationId{
		VendorId:          ptrUnsigned32(10415),
		AuthApplicationId: ptrUnsigned32(16777252),
	...
	request.WildcardedImpu = ptrUTF8String("452040000000010")
	request.RepositoryDataId = []*RepositoryDataID{
		&RepositoryDataID{
			ServiceIndication: models_base.OctetString([]byte{0x01, 0x02, 0x03}),
			SequenceNumber:    models_base.Unsigned32(1),
		},
	}
	request.ServiceIndication = []models_base.OctetString{models_base.OctetString([]byte{0x01, 0x02, 0x03})}
	request.Drmp = ptrEnumerated(1)
	request.SupportedFeatures = []*SupportedFeatures{
		&SupportedFeatures{
			VendorId:      models_base.Unsigned32(10415),
	...
	// Create Answer message
	answer := NewPushNotificationAnswer()
	answer.SessionId = models_base.UTF8String("client.example.com;1234567890;1")
	answer.VendorSpecificApplicationId = &VendorSpecificApplicationId{
		VendorId:          ptrUnsigned32(10415),
		AuthApplicationId: ptrUnsigned32(16777252),
...
// TestSN_Pair_PCAP tests PCAP file generation for SN request-response pair
func TestSN_Pair_PCAP(t *testing.T) {
	// Create testdata directory
	if err := os.MkdirAll("testdata", 0755); err != nil {
		t.Fatalf("Failed to create testdata directory: %v", err)
	}
	...
	// Create pcap file path
	pcapFile := filepath.Join("testdata", "test_snr_sna.pcap")
	// PCAP files are kept for Wireshark analysis

	// Create Request message
	request := NewSubscribeNotificationsRequest()
	request.SessionId = models_base.UTF8String("client.example.com;1234567890;1")
	request.VendorSpecificApplicationId = &VendorSpecificApplicationId{
		VendorId:          ptrUnsigned32(10415),
		AuthApplicationId: ptrUnsigned32(16777252),
	...
	request.SubsReqType = models_base.Enumerated(1)
	request.DataReference = []models_base.Enumerated{models_base.Enumerated(1)}
	request.ServiceIndication = []models_base.OctetString{models_base.OctetString([]byte{0x01, 0x02, 0x03})}
	request.IdentitySet = ptrEnumerated(1)
	request.ExpiryTime = ptrTime(models_base.Time(time.Now()))
	request.DsaiTag = []models_base.OctetString{models_base.OctetString([]byte{0x01, 0x02, 0x03})}
	request.Drmp = ptrEnumerated(1)
	request.DestinationHost = ptrDiameterIdentity("server.example.com")
	request.UserName = ptrUTF8String("452040000000010")
	request.WildcardedImpu = ptrUTF8String("452040000000010")
	request.SendDataIndication = ptrEnumerated(1)
	request.OneTimeNotification = ptrEnumerated(1)
	request.SupportedFeatures = []*SupportedFeatures{
		&SupportedFeatures{
			VendorId:      models_base.Unsigned32(10415),
			FeatureListId: models_base.Unsigned32(1),
	...
	// Create Answer message
	answer := NewSubscribeNotificationsAnswer()
	answer.SessionId = models_base.UTF8String("client.example.com;1234567890;1")
	answer.VendorSpecificApplicationId = &VendorSpecificApplicationId{
		VendorId:          ptrUnsigned32(10415),
		AuthApplicationId: ptrUnsigned32(16777252),
...
// TestUD_Pair_PCAP tests PCAP file generation for UD request-response pair
func TestUD_Pair_PCAP(t *testing.T) {
	// Create testdata directory
	if err := os.MkdirAll("testdata", 0755); err != nil {
		t.Fatalf("Failed to create testdata directory: %v", err)
	}
	...
	// Create pcap file path
	pcapFile := filepath.Join("testdata", "test_udr_uda.pcap")
	// PCAP files are kept for Wireshark analysis

	// Create Request message
	request := NewUserDataRequest()
	request.SessionId = models_base.UTF8String("client.example.com;1234567890;1")
	request.VendorSpecificApplicationId = &VendorSpecificApplicationId{
		VendorId:          ptrUnsigned32(10415),
		AuthApplicationId: ptrUnsigned32(16777252),
	...
	request.ServiceIndication = []models_base.OctetString{models_base.OctetString([]byte{0x01, 0x02, 0x03})}
	request.IdentitySet = ptrEnumerated(1)
	request.RequestedDomain = ptrEnumerated(1)
	request.CurrentLocation = ptrEnumerated(1)
	request.DsaiTag = []models_base.OctetString{models_base.OctetString([]byte{0x01, 0x02, 0x03})}
	request.Drmp = ptrEnumerated(1)
	request.DestinationHost = ptrDiameterIdentity("server.example.com")
	request.UserName = ptrUTF8String("452040000000010")
	request.WildcardedImpu = ptrUTF8String("452040000000010")
	request.OneTimeNotification = ptrEnumerated(1)
	request.RequestedNodes = ptrUnsigned32(1)
	request.ServingNodeIndication = ptrEnumerated(1)
	request.PrePagingSupported = ptrEnumerated(1)
	request.LocalTimeZoneIndication = ptrEnumerated(1)
	request.UdrFlags = ptrUnsigned32(1)
	request.SupportedFeatures = []*SupportedFeatures{
		&SupportedFeatures{
			VendorId:      models_base.Unsigned32(10415),
			FeatureListId: models_base.Unsigned32(1),
	...
	answer.SequenceNumber = ptrUnsigned32(1)
	answer.Drmp = ptrEnumerated(1)
	answer.SupportedFeatures = []*SupportedFeatures{
		&SupportedFeatures{
			VendorId:      models_base.Unsigned32(10415),
